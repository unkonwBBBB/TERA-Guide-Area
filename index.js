...
module.exports = function Tera_Guide_Area(mod) {
	...
	function sActionStage(event) {
		...
		// DA_司令 304340
		if ([2105, 3102, 3202].includes(whichmode) && event.templateId==1000) {
			/* if (skillid==106) { // 前砸(眩晕)
				SpawnThing(event.loc, event.w, 2000, 3, 180,  80,   0,  360,  18, 180);
			} */
			if (skillid==110 && event.stage==0) { // 甜甜圈
				SpawnThing(event.loc, event.w, 3000, 3,   0,   0,   0,  360,  10, 320);
			}
			if (skillid==111 && event.stage==1) { // 跳劈
				SpawnThing(event.dest, event.w, 3000, 3,   0,   0,   0,  360,  10, 200);
			}
			if ([121, 124].includes(skillid) && event.stage==0) { // 鉴定
				event.loc = event.dest;
				event.w = event.w;
				
				SpawnPoint(event.loc, event.w, 4000, 1, 110, 300);
				
				SpawnThing(event.loc, event.w, 4000, 2, 180,  58,   0,  500,  60);
				SpawnThing(event.loc, event.w, 4000, 2, 180,  58,   0,  500, 120);
				SpawnThing(event.loc, event.w, 4000, 2, 180,  58,   0,  500, 300);
				SpawnThing(event.loc, event.w, 4000, 2, 180,  58,   0,  500, 240);
		...
		if ([3103, 3203].includes(whichmode) && event.templateId==1000 && event.stage==0) {
			if (skillid==114) { // 升天拳
				SpawnThing(event.dest, event.w, 2000, 3,   0,   0,   0,  370,  10, 250);
			}
			if (skillid==124) { // 回旋飞踢
				SpawnPoint(event.dest, event.w, 2000, 1,   0,   0);
			}
			if (skillid==143) { // 正义跳跃
				SpawnThing(event.dest, event.w, 2000, 3,   0,   0,   0,  370,  10, 250);
			}
